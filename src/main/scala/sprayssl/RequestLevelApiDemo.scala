...
trait RequestLevelApiDemo {
  ...
  def demoRequestLevelApi(host: String)(implicit system: ActorSystem): Future[String] = { // execution context for future transformation below
    import system.dispatcher // execution context for futures below

    for {
      response <- IO(Http).ask(HttpRequest(GET, Uri(s"https://$host/"))).mapTo[HttpResponse]
    } yield {
